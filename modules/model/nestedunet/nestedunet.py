...
class NestedUNet(nn.Module):
    def __init__(self, in_ch=3, out_ch=1):
        ...
        self.conv2_0 = ConvBlockNested(
            filters[1], filters[2], filters[2]
        )
        self.conv3_0 = ConvBlockNested(
            filters[2], filters[3], filters[3]
        )
        self.conv4_0 = ConvBlockNested(
            filters[3], filters[4], filters[4]
        )

        self.conv0_1 = ConvBlockNested(
            filters[0] + filters[1], filters[0], filters[0]
        )
        self.conv1_1 = ConvBlockNested(
            filters[1] + filters[2], filters[1], filters[1]
        )
        self.conv2_1 = ConvBlockNested(
            filters[2] + filters[3], filters[2], filters[2]
        )
        self.conv3_1 = ConvBlockNested(
            filters[3] + filters[4], filters[3], filters[3]
        )

        self.conv0_2 = ConvBlockNested(
            filters[0] * 2 + filters[1], filters[0], filters[0]
        )
        self.conv1_2 = ConvBlockNested(
            filters[1] * 2 + filters[2], filters[1], filters[1]
        )
        self.conv2_2 = ConvBlockNested(
            filters[2] * 2 + filters[3], filters[2], filters[2]
        )

        self.conv0_3 = ConvBlockNested(
            filters[0] * 3 + filters[1], filters[0], filters[0]
        )
        self.conv1_3 = ConvBlockNested(
            filters[1] * 3 + filters[2], filters[1], filters[1]
        )

        self.conv0_4 = ConvBlockNested(
